...
store.addMap({
    ...
    template: [{
        ...
        owners: [{
            id: '?o',
            name: rdf.NodeFactory.createVar("on")
        }]
    }],
    from: '?s a dbpedia-owl:Castle . ?s dbpedia-owl:owner ?o . ?o rdfs:label ?on . Optional { ?s foaf:depiction ?d . }'
});

// var ON = rdf.NodeFactory.createVar("on");
...
listService = new service.ListServiceTransformConcept(listService, function(searchString) {
    ...
    var searchConfig = new sparql.BestLabelConfig(langs, [vocab.rdfs.label]);
...
listService.fetchItems(searchString, 1000, 0).then(function(items) { _(items).each(function(it){ console.log(it.val); }) });